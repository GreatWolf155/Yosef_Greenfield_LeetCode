...
for index1 in range(len(height)):
    for index2 in range(len(height)):
        if abs(index1 - index2) * min([height[index1], height[index2]]) > max_volume:
            max_volume = abs(index1 - index2) * min([height[index1], height[index2]])
print(max_volume)